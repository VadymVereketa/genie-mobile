import Acne from './icons/Acne.svg';
import Aging from './icons/Aging.svg';
import Bestseller from './icons/Bestseller.svg';
import Black from './icons/Black.svg';
import Brown from './icons/Brown.svg';
import Brows from './icons/Brows.svg';
import Clean from './icons/Clean.svg';
import Cleanser from './icons/Cleanser.svg';
import Colorful from './icons/Colorful.svg';
import ColourCorrecting from './icons/ColourCorrecting.svg';
import Combination from './icons/Combination.svg';
import Comedogenic from './icons/Comedogenic.svg';
import Concealer from './icons/Concealer.svg';
import ConeTaper from './icons/ConeTaper.svg';
import Contour from './icons/Contour.svg';
import Cool from './icons/Cool.svg';
import Cream from './icons/Cream.svg';
import Creams from './icons/Creams.svg';
import CreaseResistant from './icons/CreaseResistant.svg';
import Curved from './icons/Curved.svg';
import DoubleEyelid from './icons/DoubleEyelid.svg';
import Dry from './icons/Dry.svg';
import EyebrowGel from './icons/EyebrowGel.svg';
import EyebrowPencils from './icons/EyebrowPencils.svg';
import EyelashesCat from './icons/EyelashesCat.svg';
import EyelashesDoll from './icons/EyelashesDoll.svg';
import EyelashesNatural from './icons/EyelashesNatural.svg';
import EyelashesOpen from './icons/EyelashesOpen.svg';
import Eyeliner from './icons/Eyeliner.svg';
import EyePalettes from './icons/EyePalettes.svg';
import EyePrimer from './icons/EyePrimer.svg';
import Eyes from './icons/Eyes.svg';
import Face from './icons/Face.svg';
import FacePrimer from './icons/FacePrimer.svg';
import FacialItching from './icons/FacialItching.svg';
import FalseEyelashes from './icons/FalseEyelashes.svg';
import Foundation from './icons/Foundation.svg';
import Fragrance from './icons/Fragrance.svg';
import Freckles from './icons/Freckles.svg';
import Fresh from './icons/Fresh.svg';
import Full from './icons/Full.svg';
import Glossy from './icons/Glossy.svg';
import Highlighter from './icons/Highlighter.svg';
import HighShine from './icons/HighShine.svg';
import HoodedEyes from './icons/HoodedEyes.svg';
import Hydrating from './icons/Hydrating.svg';
import Hydration from './icons/Hydration.svg';
import Illuminating from './icons/Illuminating.svg';
import Indiebrands from './icons/Indiebrands.svg';
import LashGlue from './icons/LashGlue.svg';
import Light from './icons/Light.svg';
import LipGloss from './icons/LipGloss.svg';
import LipPlumper from './icons/LipPlumper.svg';
import Lips from './icons/Lips.svg';
import LipStain from './icons/LipStain.svg';
import Lipstick from './icons/Lipstick.svg';
import Liquid from './icons/Liquid.svg';
import LongLasting from './icons/LongLasting.svg';
import MakeUp from './icons/MakeUp.svg';
import Mascara from './icons/Mascara.svg';
import MascaraBlack from './icons/MascaraBlack.svg';
import MascaraBlue from './icons/MascaraBlue.svg';
import MascaraBrown from './icons/MascaraBrown.svg';
import MascaraDark from './icons/MascaraDark.svg';
import MascaraNatural from './icons/MascaraNatural.svg';
import MascaraTransparent from './icons/MascaraTransparent.svg';
import Mask from './icons/Mask.svg';
import Matte from './icons/Matte.svg';
import Mature from './icons/Mature.svg';
import Medium from './icons/Medium.svg';
import Moisturizer from './icons/Moisturizer.svg';
import Monolid from './icons/Monolid.svg';
import Natural from './icons/Natural.svg';
import Neutral from './icons/Neutral.svg';
import NowTrending from './icons/NowTrending.svg';
import OilControl from './icons/OilControl.svg';
import Oily from './icons/Oily.svg';
import Parabens from './icons/Parabens.svg';
import Pencil from './icons/Pencil.svg';
import Phthalates from './icons/Phthalates.svg';
import Pigmentation from './icons/Pigmentation.svg';
import Powder from './icons/Powder.svg';
import Radiant from './icons/Radiant.svg';
import Redness from './icons/Redness.svg';
import Rosacea from './icons/Rosacea.svg';
import Satin from './icons/Satin.svg';
import Sensitive from './icons/Sensitive.svg';
import Separation from './icons/Separation.svg';
import Serum from './icons/Serum.svg';
import SkinCare from './icons/SkinCare.svg';
import SmudgeProof from './icons/SmudgeProof.svg';
import Stick from './icons/Stick.svg';
import Sulfates from './icons/Sulfates.svg';
import Sun from './icons/Sun.svg';
import SuperVolume from './icons/SuperVolume.svg';
import Swelling from './icons/Swelling.svg';
import Thickening from './icons/Thickening.svg';
import Tint from './icons/Tint.svg';
import TopRated from './icons/TopRated.svg';
import UnderEyeCorrector from './icons/UnderEyeCorrector.svg';
import Warm from './icons/Warm.svg';
import Waterproof from './icons/Waterproof.svg';
import WaterResistant from './icons/WaterResistant.svg';
import White from './icons/White.svg';
import BobbiBrown from './images/BobbiBrown.png';
import BrightColors from './images/BrightColors.png';
import Chanel from './images/Chanel.png';
import CoolTonedNeutrals from './images/CoolTonedNeutrals.png';
import Dior from './images/Dior.png';
import FullGlam from './images/FullGlam.png';
import Givenchy from './images/Givenchy.png';
import Loreal from './images/Loreal.png';
import Mac from './images/Mac.png';
import Maxfactor from './images/Maxfactor.png';
import Oranges from './images/Oranges.png';
import Pinks from './images/Pinks.png';
import SomeGlam from './images/SomeGlam.png';
import VeryNatural from './images/VeryNatural.png';
import WarmTonedNeutrals from './images/WarmTonedNeutrals.png';
import Ysl from './images/Ysl.png';

export const BoxIcons = {
  Eyeliner,
  FalseEyelashes,
  LashGlue,
  Mascara,
  EyePalettes,
  UnderEyeCorrector,
  MakeUp,
  Glossy,
  HighShine,
  Hydrating,
  Stick,
  Tint,
  SkinCare,
  Brows,
  Eyes,
  Face,
  Lips,
  Mask,
  Cleanser,
  Clean,
  ColourCorrecting,
  Full,
  Illuminating,
  Light,
  Medium,
  Satin,
  Sun,
  Moisturizer,
  Serum,
  Bestseller,
  Acne,
  Matte,
  Fresh,
  Radiant,
  Aging,
  Combination,
  Comedogenic,
  Dry,
  FacialItching,
  Fragrance,
  Pencil,
  Freckles,
  Hydration,
  Indiebrands,
  Mature,
  NowTrending,
  OilControl,
  Oily,
  MascaraDark,
  MascaraNatural,
  MascaraTransparent,
  Parabens,
  Phthalates,
  Pigmentation,
  Redness,
  Rosacea,
  Sensitive,
  Sulfates,
  Swelling,
  TopRated,
  Warm,
  Cool,
  Neutral,
  EyePrimer,
  EyebrowGel,
  EyebrowPencils,
  LipPlumper,
  LipStain,
  Lipstick,
  LipGloss,
  Foundation,
  Concealer,
  EyelashesNatural,
  EyelashesCat,
  EyelashesDoll,
  EyelashesOpen,
  Powder,
  Contour,
  Highlighter,
  FacePrimer,
  Creams,
  LongLasting,
  SmudgeProof,
  Waterproof,
  WaterResistant,
  CreaseResistant,
  Colorful,
  Black,
  Brown,
  White,
  DoubleEyelid,
  HoodedEyes,
  Monolid,
  Liquid,
  Cream,
  ConeTaper,
  Curved,
  MascaraBlack,
  MascaraBrown,
  MascaraBlue,
  Natural,
  Separation,
  SuperVolume,
  Thickening,
};

export const BoxImages = {
  Loreal,
  BobbiBrown,
  Chanel,
  Dior,
  Givenchy,
  Mac,
  Maxfactor,
  Ysl,
  BrightColors,
  FullGlam,
  Oranges,
  Pinks,
  SomeGlam,
  VeryNatural,
  WarmTonedNeutrals,
  CoolTonedNeutrals,
};

export type BoxIconsType = keyof typeof BoxIcons;
export type BoxImagesType = keyof typeof BoxImages;
